...
from src.core.models import ViralSequence, EscapeScore
...
class EscapeAgent(BaseAgent):
    ...
    def execute(self, input_data: dict) -> dict:
        ...
        for pred in predictions:
            if pred.escape_impact > 0.3:
                concerning_predictions.append({
                    ...
                    "combined_risk": round(pred.escape_impact * pred.probability, 4),
                })
        concerning_predictions.sort(key=lambda x: x["combined_risk"], reverse=True)

        # Summary statistics
        scores_sorted = sorted(escape_scores, key=lambda x: x.overall_escape, reverse=True)
        ...
        result = {
            ...
            "highest_escape_lineage": scores_sorted[0].lineage if scores_sorted else None,
            "average_escape": round(
                sum(s.overall_escape for s in escape_scores) / max(len(escape_scores), 1), 4
            ),
        }

        return result

    def _build_risk_matrix(self, escape_scores: list[EscapeScore],
        ...
        if high_escape_predictions:
            projected_escape_increase = sum(p.escape_impact * p.probability for p in high_escape_predictions[:10])
            matrix["projected_risk"] = [{
                ...
                "assessment": "ELEVATED" if projected_escape_increase > 0.3 else "MODERATE",
            }]

        # Overall assessment
        max_escape = max((s.overall_escape for s in escape_scores), default=0)
        if max_escape > 0.7:
            matrix["overall_assessment"] = "CRITICAL — Significant immune evasion. Vaccine update urgently needed."
        elif max_escape > 0.5:
            matrix["overall_assessment"] = "HIGH — Substantial escape detected. Monitor and prepare updated vaccines."
        elif max_escape > 0.3:
            matrix["overall_assessment"] = "MODERATE — Partial escape. Current vaccines retain meaningful efficacy."
        else:
            matrix["overall_assessment"] = "LOW — Minimal escape. Current vaccines remain effective."

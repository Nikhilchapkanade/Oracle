...
with st.sidebar:
    ...
    if st.button("🚀 Run ORACLE Pipeline", type="primary", use_container_width=True,
    ...
    if st.session_state.pipeline_result:
        ...
        if status == "completed":
            ...
            st.caption(f"Duration: {st.session_state.pipeline_result.get('total_duration_seconds', 0):.1f}s")
        else:
            st.error(f"❌ Pipeline failed")

    st.markdown("---")
    st.markdown("### 📊 Quick Stats")
    if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
...
with tabs[0]:
    if not st.session_state.pipeline_result or st.session_state.pipeline_result["status"] != "completed":
        ...
        **Click "Run ORACLE Pipeline"** in the sidebar to start.
        ...
        for i, (lineage, data) in enumerate(LINEAGE_DEFINITIONS.items()):
            with cols[i % 4]:
                risk_color = "#ff4444" if data.get("escape", 0) > 0.6 else "#ffcc00" if data.get("escape", 0) > 0.3 else "#44ff44"
                st.markdown(f"""
        ...
        exec_summary = report.get("executive_summary", "Run the pipeline to generate a summary.")
        ...
        if alerts:
            ...
            for alert in alerts:
                ...
                css_class = "alert-high" if level == "HIGH" else "alert-warning" if level == "WARNING" else "alert-info"
...
with tabs[1]:
    st.markdown('<div class="section-header">📡 Real-Time Variant Tracker</div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
        ...
        if region_dist:
            ...
            fig_region = go.Figure(data=[go.Bar(
                ...
                marker_color=['#ff6b6b', '#ffd93d', '#6bcb77', '#4d96ff', '#845ec2', '#ff9671', '#d65db1'],
            )])
...
with tabs[2]:
    st.markdown('<div class="section-header">🔮 Mutation Forecast</div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
        ...
        if pred_summary:
            ...
            df_pred["fitness_impact"] = df_pred["fitness_impact"].apply(lambda x: f"{x:+.3f}")
            df_pred["escape_impact"] = df_pred["escape_impact"].apply(lambda x: f"{x:.3f}")
        ...
        if trajectory:
            ...
            for step in trajectory:
                with st.expander(f"Step {step['step']} — {step['timeframe_days']} days"):
                    st.write(f"Cumulative mutations: {step['cumulative_mutations']}")
                    st.write(f"Estimated fitness change: {step['estimated_fitness_change']:+.3f}")
...
with tabs[3]:
    st.markdown('<div class="section-header">🌳 Phylogenetic Analysis</div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
        ...
        col1, col2 = st.columns(2)
        with col1:
            fig_tree = go.Figure(data=[go.Scatter(
        ...
        with col2:
            fig_muts = go.Figure(data=[go.Bar(
...
with tabs[4]:
    st.markdown('<div class="section-header">🛡️ Immune Escape Analysis</div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
        ...
        if escape_summary:
            ...
            for cls in ["class1", "class2", "class3", "class4"]:
                cls_scores = [s.get("class_scores", {}).get(cls, 0) for s in escape_summary]
                fig_escape.add_trace(go.Bar(
                    name=f"Ab {cls.title()}",
                    x=lineages,
                    y=cls_scores,
                ))
...
with tabs[5]:
    st.markdown('<div class="section-header">💉 Vaccine Candidate Design</div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
        ...
        if candidate_comp:
            ...
            for i, c in enumerate(top3):
                values = [c['immunogenicity'], c['breadth'], c['stability'], c['escape_resistance']]
            ...
            for col in ['immunogenicity', 'breadth', 'stability', 'escape_resistance', 'overall_score']:
            ...
            if rec:
                ...
                st.success(f"**{rec.get('recommended_candidate', 'N/A')}** | "
...
with tabs[6]:
    st.markdown('<div class="section-header">🤖 Agent Pipeline Monitor</div>', unsafe_allow_html=True)
    ...
    if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
        ...
        with col1:
            ...
            st.write(f"**Total Duration:** {result.get('total_duration_seconds', 0):.2f}s")
            st.write(f"**Status:** ✅ Completed")
        with col2:
            metrics = result.get("metrics", {})
            st.write(f"**Sequences Processed:** {metrics.get('sequences_processed', 0)}")
            st.write(f"**Mutations Detected:** {metrics.get('mutations_detected', 0)}")
            st.write(f"**Predictions Generated:** {metrics.get('predictions_generated', 0)}")
    else:
...
with tabs[7]:
    st.markdown('<div class="section-header">📈 MLOps Dashboard</div>', unsafe_allow_html=True)

    mlops = st.session_state.mlops_pipeline

    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("🔄 Run Drift Detection", type="secondary", use_container_width=True):
            if st.session_state.pipeline_result and st.session_state.pipeline_result["status"] == "completed":
                surveillance = st.session_state.pipeline_result["all_results"].get("surveillance", {})
                lineage_dist = surveillance.get("lineage_distribution", {})
                mlops.drift_detector.set_reference({"JN.1": 50, "XBB.1.5": 30, "BA.5": 20})
    ...
    with col2:
        if st.button("🔧 Trigger Retraining", type="secondary", use_container_width=True):
            if st.session_state.pipeline_result:
                surveillance = st.session_state.pipeline_result["all_results"].get("surveillance", {})
                result = mlops.retrain(surveillance, "evolution_forecaster",
    ...
    if models:
        ...
        for model_name, info in models.items():
            st.write(f"**{model_name}** — {info['total_versions']} versions | Latest: {info['latest']}")
            st.json(info["stages"])
